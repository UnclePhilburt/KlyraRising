...
namespace Synty.AnimationBaseLocomotion.Samples
{
    public class SampleCameraController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            _syntyCamera.localPosition = new Vector3(0, 0, _cameraDistance * -1);
        ...
        private void Update()
        {
            ...
            // Apply rotation first so we can use the camera's orientation for the offset
            transform.eulerAngles = new Vector3(_newAngleX, _newAngleY, 0);

            // Calculate pivot position with offset applied in camera space (for over-shoulder effect)
            Vector3 targetPivot = _playerTarget.position;
            targetPivot += transform.right * _cameraHorizontalOffset;
            targetPivot += transform.up * _cameraHeightOffset;

            _newPosition = Vector3.Lerp(_lastPosition, targetPivot, positionalFollowSpeed * Time.deltaTime);
            transform.position = _newPosition;

            // Camera is now positioned straight back from the offset pivot
            _syntyCamera.localPosition = new Vector3(0, 0, _cameraDistance * -1);
            _syntyCamera.localEulerAngles = new Vector3(_cameraTiltOffset, 0f, 0f);
